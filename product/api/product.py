...
def generate_action(parts_list: list):
    return ", ".join(
        [
            "{} - ({}) - {}".format(part["part_name"], part["qty"], part["brand"])
...
def update_product_stock(request, *args, **kwargs):
    ...
    for part in parts:
        ...
        if not queryset:
            ...
            if product_serializer.is_valid():
                ...
                error_dict = {
                    error: product_serializer.errors[error][0]
                    for error in product_serializer.errors
                }
                return Response(error_dict, status=status.HTTP_409_CONFLICT)
        else:
            queryset.update(
                remaining_stock=F("remaining_stock") + quantity, **inbound_dict
            )
    ...
    return Response(
        {
            "message": f"Remaining stocks increased in {part_list}. Invoice Number: {reference_number}"
        }
    )
...
def update_inbound_history(body):
    ...
    error_dict = {
        error: inbound_serializer.errors[error][0]
        for error in inbound_serializer.errors
    }
...
def outbound_product(request, *args, **kwargs):
    ...
    for part_item in parts:
        ...
        queryset = Product.objects.filter(
            part=part_id, warehouse=body.get("warehouse_from")
        )
        if not queryset:
            return Response(
        ...
        remaining_stocks = (
            Product.objects.filter(part=part_id, warehouse=body.get("warehouse_from"))
            .values("remaining_stock")
            .first()["remaining_stock"]
        )
        print(
            f"Remaining Stocks of {part_id} is {remaining_stocks}, quantity is {quantity} "
        )

        if remaining_stocks < quantity:
            return Response(
                {"message": "value larger than stocks"},
    ...
    return Response(
        {
            "message": f"Remaining stocks decreased in {parts}. Invoice Number: {reference_number}"
        }
    )
...
def update_outbound_history(body):
    ...
    error_dict = {
        error: outbound_serializer.errors[error][0]
        for error in outbound_serializer.errors
    }
    return Response(error_dict, status=status.HTTP_409_CONFLICT)
...
def product_search_view(request, *args, **kwargs):
    ...
    if (filter_by and filter_id) or search_key:
        ...
        if search_key:
            filter_dict["part__part__contains"] = search_key

    result = {}
    if not (current_page and page_size):
        if filter_dict:
            result["data"] = ReturnProductSerializer(
                Product.objects.filter(**filter_dict).all().order_by(sort_by), many=True
            ).data
        else:
            result["data"] = ReturnProductSerializer(
                Product.objects.filter().all().order_by(sort_by), many=True
            ).data
        return Response(result)

    if filter_dict:
        p = Paginator(
            Product.objects.filter(**filter_dict).order_by(sort_by), page_size
        )
    else:
...
class InboundHistoryCreateAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": f"InboundHistory {body.get('inbound_history')} successfully created"
                }
            )
...
class InboundHistoryUpdateAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": f"InboundHistory {inbound_history_name} successfully updated"}
        )
...
def inbound_history_search_view(request, *args, **kwargs):
    ...
    if not (current_page and page_size):
        if filter_dict:
            result["data"] = (
                InboundHistory.objects.filter(**filter_dict)
                .all()
                .order_by(sort_by)
                .values()
            )
            ...
            result["data"] = (
                InboundHistory.objects.filter().all().order_by(sort_by).values()
            )
        return Response(result)

    if filter_dict:
        p = Paginator(
            InboundHistory.objects.filter(**filter_dict).all().order_by(sort_by),
            page_size,
        )
    else:
        p = Paginator(
            InboundHistory.objects.filter().all().order_by(sort_by), page_size
        )
    ...
    result["metadata"]["numPages"] = p.num_pages
    result["data"] = p.page(current_page).object_list
    result["data"] = InboundHistorySerializer(
        p.page(current_page).object_list, many=True
    ).data
